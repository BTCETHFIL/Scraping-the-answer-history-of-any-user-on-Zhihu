# 知乎回答爬虫 (zhihu-crawler) — v2.0

抓取指定知乎用户的回答历史，导出为结构化 Markdown 文件。每篇回答截图以 Base64 内嵌，文件完全自包含，跨电脑可直接查看。
...
- **已抓取用户列表** — 自动生成 `已抓取用户列表.md`，包含每个用户的基本信息、抓取次数、累计回答数、本地文件校验
- **目录自动合并** — 同一用户昵称变化导致多个目录时自动合并，避免重复
- **昵称自动清洗** — 抓取时过滤混入昵称的个人简介/签名，保持目录名干净
...
> 目录名格式 `{昵称}_{用户ID}`，同一用户昵称变化时自动合并历史目录到新目录。
